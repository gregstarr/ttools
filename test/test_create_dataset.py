...
map_periods = [np.timedelta64(5, 'm'), np.timedelta64(30, 'm'), np.timedelta64(1, 'h'), np.timedelta64(2, 'h')]
...
def test_assemble_args(times, map_period):
    ...
    args = create_dataset.assemble_binning_args(mlat, mlt, tec, times, bins, map_period)
    if np.ceil((times[-1] - times[0]) / map_period) == (times[-1] - times[0]) / map_period:
        assert len(args) == (times[-1] - times[0]) / map_period + 1
    else:
...
def test_process_month(madrigal_data_dir, map_period):
    """not that good of a test: wait for bugs and rewrite
    """
    ...
    times, tec, n, std, ssmlon = create_dataset.process_month(month, mlat, mlon, converter, bins, map_period,
                                                              madrigal_data_dir)
